...
categories: [machine-learning, world-models, research]
tags: [jepa, lewm, billiards, world-model, apple-silicon, pytorch]
excerpt: "I applied LeWM to 2D billiards and discovered two sequential bottlenecks for JEPA-based planning: a representational eviction failure mode under frame stacking, and an architectural fix (auxiliary state supervision) that fully recovers both position and velocity encoding. Seven experiments, two findings."
image: /assets/images/combo_b_planning.gif
math: true
---
...
![Planning Results](/assets/images/planning_results.png)
*Start frame, goal frame, and execution result for both combos.*

![Combo B Planning](/assets/images/combo_b_planning.gif)
...
![t-SNE Latent Space](/assets/images/tsne_billiards.png)
...
## Update: Three More Experiments

After publishing the original post, the velocity encoding gap pointed to two natural fixes: give the encoder explicit motion signal, and force it to preserve position. We ran three more experiments to test these hypotheses.

**Summary across all four model variants (192-dim CLS probe):**
